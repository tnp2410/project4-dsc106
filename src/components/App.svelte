<main>
  ...
  <p>
  We began the project with data cleaning the Pokemon dataset by dropping unnecessary columns, fixing N/A values, and converting types into a list. The next step began by looking at examples of how to implement our visualization. Our original plan was to implement a radar chart to display how effective different types of pokemon are against each other. After spending a few hours, we kept running into errors because we couldn’t figure out how to get the visualization to display properly. We decided to shift gears and approach our project by implementing a Zoomable sunburst and changing our goals to exploring different Pokemon types sorted by generations. 
  </p>
  <p>
  The most challenging part of our project design was getting the visualization to load onto the website by connecting all the moving parts such as importing d3. Another difficulty we faced was correctly importing the data as we intended because there was confusion in figuring out how to use the proper variables in each file. Although this is just a prototype, I anticipate that getting the tool tip to work will be challenging because what is displayed is based on where the cursor is on the website which will take a while to implement. Once we figure out the finishing touches such as color coding, the visualization will look complete. 
  </p>
